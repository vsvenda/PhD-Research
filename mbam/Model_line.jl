...
import ..System_info: yline, a, Bc, nl, nr, nbus, nbr, PFlineIndex, ImlineIndex, fbus, tbus, meas_data, meas_var, meas_type, m_outage, refbus, x_start;
...
var_out = 1e2; # variance for measurements not taken into account

# Reducing the original network **********************************************************
bus_unob = [10; 11; 12]; # initial unobservable buses

line_obs = zeros(length(yline));  # vector denoting observable / unobservable lines (initially all observable)
...
if !isempty(bus_unob)
	...
	for i = 1:length(yline) # denote observable lines 
		if !(((nl[i] in bus_unob) && (nr[i] in neigh_bus)) || ((nl[i] in neigh_bus) && (nr[i] in bus_unob)) || ((nl[i] in bus_unob) && (nr[i] in bus_unob)))
...
println("\nObserved network part line information:")
for i=1:length(nl)
	cnt = 1;
	if line_obs[i] == 0
		println(cnt , ". " , i , " - Line " , line_ID[i] , " Lead bus : " , nl[i] , " Rec. bus : " , nr[i]);
		cnt += 1;
	end
end
...
function h(x::Vector{T}) where T <: Real
	...
	for i = 1:length(bus_ID)
		push!(theta, x_start[bus_ID[i]]);
		push!(V, x_start[bus_ID[i]+nbus]);
	end	
	# ******************************************************************************************************************************
...
xi_line = [1.0];
for i = 2:length(yline)-length(y_oc)-length(y_sc)-count(i->i==1,line_obs)
	push!(xi_line,1.0);
end
...
for i = 1:(2*npi+2*npf+nim)	# Voltage measurements variances are stored last in meas_var
	...
	elseif !(fbus[i] in bus_unob) && !(fbus[i] in neigh_bus) # ADD TBUS (PF measurement going towards neigh. buses from outside the observed area) ???
		push!(weights,(1/sqrt(var_out))); # measurement outside of the observed area
	else
