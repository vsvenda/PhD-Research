...
bus_unob = []; # initial unobservable buses
...
function h(x::Vector{T}) where T <: Real
	...
	for i = 1:length(y_sc)
		...
		if nr[y_sc[i]] in bus_ID
			deleteat!(bus_ID,findall(x->x==y_sc[i],bus_ID)); # merge receiving to leading bus (remove receiving bus)
		end
	...
	for i = 1:length(yline)	# forming off diagonal elements
		...
		if !isempty(nl_sc) && ((nr[i] in nr_sc) || (nl[i] in nr_sc)) # branches connected to receving bus of short circuit branch
