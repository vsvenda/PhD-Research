...
set f [open Results_300_Bus_Area1_Part2.tr w]
$ns trace-all $f

$ns use-scheduler Simulink

# ******** Create nodes ********
for {set i 0} {$i < 78} {incr i 1} {
set n($i) [$ns node]
}
...
for {set i 0} {$i < 77} {incr i 1} {
$ns duplex-link $n($i) $n(77) 2Mb [$delay_RTU value]ms DropTail
}

# ******** Set up transportation level connections ********
# --- EMS ---
set null_n(77) [new Agent/Null]
    $ns attach-agent $n(77) $null_n(77)
...
for {set i 0} {$i < 77} {incr i 1} {
$ns connect $udp_n($i) $null_n(77)
    $udp_n($i) set fid_ $i
}

for {set i 0} {$i < 77} {incr i 1} {
$ns queue-limit $n($i) $n(77) 2
}
...
for {set i 0} {$i < 77} {incr i 1} {
$ns link-lossmodel $em_RTU $n($i) $n(77)
}
